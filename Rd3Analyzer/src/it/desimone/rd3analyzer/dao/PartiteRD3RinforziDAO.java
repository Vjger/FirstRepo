...
public class PartiteRD3RinforziDAO extends AbstractDAO {

	public PartiteRD3RinforziDAO(Connection connection) {
		super(connection);
	}

	private static final String INSERT = 
			"INSERT INTO PartiteRD3Rinforzi "
			+ "(idPartita, logId, ColoreGiocatore, Time, TerritorioRinforzato, NumeroRinforzi) "
			+ "VALUES (?,?,?,?,?,?)"; 
	
	private static final String UPDATE = 
			"UPDATE PartiteRD3Rinforzi "
			+ "SET ColoreGiocatore = ?, Time = ?, TerritorioRinforzato = ?, NumeroRinforzi = ? "
			+ "WHERE idPartita = ? AND logId = ?";
	
	
	public int inserisciPartita(PartiteRD3Rinforzi partita){
		...
		try {						
			...
			preparedStatement.setString(5, partita.getTerritorioRinforzato());
			preparedStatement.setInt(6, partita.getNumeroRinforzi());
	...
	public int updatePartita(PartiteRD3Rinforzi partita){
		//MyLogger.getLogger().finest("[BEGIN]: "+partita);
		boolean psOpened = false;
		int righeInserite = 0;
		PreparedStatement preparedStatement = null;
		try {						
			...
			preparedStatement.setString(3, partita.getTerritorioRinforzato());
			preparedStatement.setInt(4, partita.getNumeroRinforzi());
