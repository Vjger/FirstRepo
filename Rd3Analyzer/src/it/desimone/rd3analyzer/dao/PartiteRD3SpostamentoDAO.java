...
public class PartiteRD3SpostamentoDAO extends AbstractDAO {

	public PartiteRD3SpostamentoDAO(Connection connection) {
		super(connection);
	}

	private static final String INSERT = 
			"INSERT INTO PartiteRD3Spostamento "
			+ "(idPartita, logId, ColoreGiocatore, Time, TerritorioDiProvenienza, TerritorioDiArrivo, NumeroArmateSpostate) "
			+ "VALUES (?,?,?,?,?,?,?)"; 
	
	private static final String UPDATE = 
			"UPDATE PartiteRD3Spostamento "
			+ "SET ColoreGiocatore = ?, Time = ?, TerritorioDiProvenienza = ?, TerritorioDiArrivo = ?, NumeroArmateSpostate = ? "
			+ "WHERE idPartita = ? AND logId = ?";
	

	public int inserisciPartita(PartiteRD3Spostamento partita){
		...
		try {						
			...
			preparedStatement.setString(5, partita.getTerritorioProvenienza());
			preparedStatement.setString(6, partita.getTerritorioArrivo());
			preparedStatement.setInt(7, partita.getNumeroArmateSpostate());
	...
	public int updatePartita(PartiteRD3Spostamento partita){
		//MyLogger.getLogger().finest("[BEGIN]: "+partita);
		boolean psOpened = false;
		int righeInserite = 0;
		PreparedStatement preparedStatement = null;
		try {						
			...
			preparedStatement.setString(3, partita.getTerritorioProvenienza());
			preparedStatement.setString(4, partita.getTerritorioArrivo());
			preparedStatement.setInt(5, partita.getNumeroArmateSpostate());
